...
namespace WebAPI.Models
{
    public static class Process
    {
        ...
        public static async Task<ProcessResult> RunInTransactionAsync(Func<Task> action, DbOperator dbOperator)
        {
            using (var transaction = dbOperator.Database.BeginTransaction())
            {
                try
                {
                    await action();
